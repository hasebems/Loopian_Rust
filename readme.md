...
- noteの音価表現
    ...
    - d.. / d~~ : ドを基準音価の３倍（もう２回）伸ばす 
        ...
        - do|o| は２小節伸ばす
    - [hd,qd,ed,vd,'"d] : h:二分音符、q:四分音符、' or e:八分音符、" or v:十六分音符、'":三十二分音符
...
- {*chord*} : Composition の書式
    ...
    - {} : 全データ削除
    - {.....//} : 最後に // が二つあった場合（あるいは "/END")、phrase も composition も繰り返さず、音が消える
...
- 'set.path=xxx' : load の下にあるディレクトリを指定
