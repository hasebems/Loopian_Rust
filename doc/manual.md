...
- [Composition 指定](#composition-指定)
    ...
    - [コードの根音(root)指定方法](#コードの根音root指定方法)
    - [コード記述方法](#コード記述方法)
    - [コード・スケールの種類](#コードスケールの種類)
    - [スケールのダイレクト指定機能](#スケールのダイレクト指定機能)
- [Phrase / Composition の共通仕様、及び連携機能](#phrase--composition-の共通仕様及び連携機能)
    - [Dynamic-Pattern](#dynamic-pattern)
    - [Part指定書法](#part指定書法)
    - [Phrase の parallel 指定](#phrase-の-parallel-指定)
    - [Variation機能](#variation機能)
- [Phrase のその他の機能](#phrase-のその他の機能)
    - [cluster_memory機能](#cluster_memory機能)
...
## Loopianとは
Loopian は、Live Coding などで使うために開発している、テキスト入力によるピアノ音色用シーケンサである。
以下の特徴を持つ
...
- part : phrase/composition は、独立した複数のセットの Loop 再生が可能である。そのphraseとcompositionがセットになったものを part と呼ぶ。
- 音価 : 音符の長さ
...
- 音符の記述
    - `,` : 音符情報の区切りを表す。１小節分を超えたら捨てられる。区切りが連続すると休符が省略されたとみなす
    - `|` or `/` : 小節区切りを表す。区切りが連続すると休符が省略されたとみなす
    - `x` : 休符
    - `dms` : ドミソの同時演奏 (各音を続けて書く)
        - 左側から低い順に音程を記述する
        - `[d+d]` : ドとその１オクターブ上のド(相対オクターブ指定の場合)
    - `d*4` : ドを４回連続して発音


- オクターブの記述
    - オクターブは、`-`、`+`を、音符の先頭に付けることによって表現する
        - `-d` : 下のド
        - `+d` : 上のド
        - `--d`: 二つ下のド
        - `++d`: 二つ上のド
    - オクターブを記述する方法の設定を `set.input` で変更できる(デフォルトはinput(closer))
    ...
    - 相対オクターブ指定(input(upcloser))では、上側１オクターブ分(1..11)に行く場合何も表記しない。
...
- `w`:三十二分音符
- `w(N)`:任意の数値を直接指定することで、装飾音符などを表現する(Nはtick数を表す数値。なお四分音符は480tickである)
    - `[w(24)d,r,m,f,s]` : 十六分音符の間に5つの音符d,r,m,f,sを等間隔で演奏する 
- `[3ed,r,m]` : eの前に3を書くと、一拍三連の音価になる（八分音符で三連符を記述したことと同じ）
    - 同様に、5連符が可能
    - 同様に、3q で二拍三連
...
- **<注意!>** 弱起のフレーズの次に再生するフレーズは、最後の１小節になってから入力すると弱起のフレーズが始まってしまい再生が間に合わない。従って、それより前の小節で入力するか、弱起のフレーズに `//` を入れ、繰り返さないようにするなど注意が必要である
...
- **音符変調関数**(note modulation function)
    ...
    - `move(n)` `mv(n)` : 入力された音符を n=-60..+60半音分（最大で）、上下移動させる
    - `copy(n)` `cp(n)` : 入力された音符に対して n=-60..+60半音（最大で）の位置に、新しい音符を追加する
- **音楽表現関数**(musical expression function)
    - `dyn(f)` : f,mf,mp,p,pp を引数に入れ、フレーズの音量指定
    - `dmp(off)` : off: 和音指定時でも Pedal Off指定
    - `stacc(50)` : 音価を半分にする（1-100まで調整可能）
    - `legato(120)` : 音価を20%増やす（100-200まで調整可能）
    - `para()` : 和音変換時、parallel 指定する。（全フレーズの parallel と同じ）
...
### コード記述方法
- `X` : コード変換は行わない(no pedal)
- `O` : コード変換は行わない(pedal)
- `I` : `dms` (Iの和音)
- `I#` : `dimisi` (数字の後に # を付けると半音高いコードになる。b は半音低くなる)
- `_ion, _dor, _lyd, _mix, _aeo` という教会旋法を指定した場合、通常のコード指定とは動作が変わる
    - `Iion` : Iを主音としたイオニアンスケール(Ionian)を意味する
    - ローマ数字と教会旋法の組み合わせが `Iion, IIdor, IVlyd, Vmix, VIaeo` の五種類のとき、現在の調のダイアトニックスケールになり、それ以外の root が指定された場合、前述した組み合わせのローマ数字の差分分だけ並行移動する。
...
## Phrase / Composition の共通仕様、及び連携機能


### Dynamic-Pattern
- Dynamic Pattern は、Phrase の中で実際の音程を指示せず、Composition で指定された和音のコード演奏、分散和音を自動生成する
- 同時に鳴る和音は `Cls()` (Cluster)、分散和音は `Arp()` (Arpeggio)と記述する
    - `C(), A()` と小文字を省略可能
    - `[hCls(),hArp()]` のように、Phrase の中に記載し、前に Pattern 全体の音価を示す
- `Cls(x,y,z)` とカッコ内をカンマで区切って、パラメータを与えることができる
    - パラメータは最大で3つで、４つ以上与えた場合以降は無視される
    - パラメータが足りない場合は、default値が適用される
    - x（一つ目のパラメータ）: 音価を表す。h,q,e,v,w およびその三連、五連、付点を選ぶことができる(default:q)
        - `o` を指定すると、どんな拍子でも、1小節分の長さを指定できる
    - y（二つ目のパラメータ）: 和音の音数(2-5)(default:4)
    - z（三つ目のパラメータ）: 最低音の位置(-6 - 7)(default:0)
    - `$C()` のように `C` の前に`$`, `$S`、`$Q`がある場合、同時和音に対するアルペジオ演奏を行う
        - `$SC` ゆっくりなアルペジオ(Slow)となる
        - `$QC` 速いアルペジオ(Quick)となる
        - `h$C()` 音価情報はアルペジオの前に置かれる
        - 下記の `Arp()` とは名称が被っているが、`$` は記譜上のアルペジオ記号のことを指している
- `Arp(x,y,z)` とあった場合、各パラメータは以下の意味になる
    - x: 音価を表す。h,q,e,v,w およびその三連、五連、付点を選ぶことができる(default:q)
    - y: u は上昇音形、dは下降、uxはクロス上昇（音程が上下上と移行）、dxはクロス下降（下上下と移行）
    - z: 最低音の位置(-6 - 7)(default:0)
...
- Phrase / Composition ともに、冒頭でパート指定することで、プロンプトのパートと違うパートも入力することができる
    - `L1.[...]`  : left1パート
    - `L.[...]` : left1,left2パート二つ同時に同じ Phrase をセット
    - `ALL.[...]` : 全パートに同じ Phrase をセット
    - `L1!.[...]` : left1パート以外全て


### Phrase 個別の音程の parallel 指定
- parallel 指定とは、和音変換時 root の分だけ Phrase を並行移動させることである
    - 例えば `{IV}` の場合だと、Phase 全体が完全四度移動した上で、和音構成音への変換が行われる
- note単位の和音変換指定が可能
    ...
    - 複数音を無変換にしたい場合、`<...>n` とする
- [`para()`](#phraseに後続する関数表記) は、フレーズ全体を `<...>p` で囲むのと同じ効果になる
    - フレーズ全体を parallel にしたい場合は `para()` を、一部の音符のみを parallel にしたい場合は `<...>p` を使う
...
- 特定の小節で再生するとき `@msr(M)=[..]` と表記し、M の部分に小節番号を記述する
...
- `!transmit`（あるいは`!t`）：ファイルからロードした内容を転送する
    - 後述する `!blk(..)` `!msr(n)` 以降の有効行は再生しない
    - 後述する `!rd(..)` の行は再生しない
...
- 冒頭が `//` になっている行はコメントとみなされ、ロードされない
- 空白行があった場合、以降に説明する `!` による記法の効力がそこで終わることを意味する
- `!msr(n)` とファイル内に書かれた行以降は、n小節の1拍目に間に合うように転送される
    - 実際には、n-1小節目の残り240tickを切ったところで転送される
    - load 中に wait があり、自動 load 中と見做される場合でも、ユーザーによる Input Window への入力は可能である
- `!rd(n): xxx` とファイル内に記載されている場合、ファイルをロード後 `!rd(n)` と Input Window に入力すると、`xxx` が自動に入力される（nは任意の数字）
- `!blk(a)`: ファイル内の行の冒頭が `!blk(a)` とある行から、次の空行、あるいは別の`!blk`,`!msr` があるところまでの行が転送される
- `!blk(a,T,S..)` : T, S と引数を二つ以上に増やすことで、ブロック内の任意の文字列を差し替えることができる
    - まず、ファイル内に `!blk(a,T)` と記述し（`a` はブロック名、`T` は差し替え変数名）、それ以降の行の差し替えるところに `T` と書く
    - コマンドで指示する際、例えば `!blk(a,R1)` と書くと、`a` ブロック内のファイル内の `T` の場所が `R1` に差し替わる
- `!msr(n)`: 入力後 `resume` とコマンドを打つと、ファイル内の行の冒頭が `!msr(n)` とある行から再生する
    - 1小節前から空の再生を始める
...
- `sync` : 次の小節の頭で、そのパートの Phrase, Composition を同期させる
    ...
    - `sync.all`   : 全パート
- `fine` : 現在の小節が終わったらシーケンス終了
    - `fine.next` : 次の小節が終わったら終了
    - `fine.beat(2)`  : 次の小節の２拍目で終了(2,3,4)
- `clear` : データの中身を消去
...
- `&&` と記述し続けてコマンドを書くことで、複数のコマンドを一行で入力することができる
    - ファイル内での記述では `&&` は使えない
...
- `set.input(fixed)` : オクターブ指定法
    ...
    - `closer` は、+-指示がない限り、前回に近い音程(-5..6/s..fi) (default)
    - `upcloser` は、+-指示がない限り、前回より1オクターブ上の範囲(1..11/d..t)
...
### 音楽表現関数の詳細仕様
  ...
  - 音楽表現関数の中でも音量や音価の変化を指示する場合、`R1.fn()` のように、Phrase を記述せずにパート名の後にピリオドで繋げて指定することも可能である
    ...
    - `R1.dyn(mf)` : R1 の音量を直ちに mf にする
        - `dyn(/f)` : 次の小節の頭で f になる（未実装）
        - `dyn(<f)` : 現在の LOOP のときの音量が f 以下の場合、この LOOP の先頭からクレシェンドして、次の小節の頭で f になる
        - `dyn(>p)` : 現在の LOOP のときの音量が p 以上の場合、この LOOP の先頭からディミニエンドして、次の小節の頭で p になる
...
- load フォルダ内のデータは以下の手順で再生する
    ...
    1. `!play` or `!p` で、ロードされたファイル内容を転送する
    1. `play` or `p` で、ロードされたデータを再生
...
- 曲データを打ち込む際は、以下の手順で行う
    ...
    1. 全パート打ち込んだら、パートごとの並びを小節順に並び替え、`!msr(M)=[]` の表記で書き直す（継時書法）
        - パート分割書法を継時書法に自動に書き換える `!cnv2tl` コマンドが利用可能
