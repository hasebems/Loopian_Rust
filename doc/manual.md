...
- `!play`（あるいは`!p`）：ファイルからロードした内容を再生する
...
- `!blk(a)`: ファイル内の行の冒頭が `!blk(a)` とある行から、次の空行、あるいは別の`!blk`,`!msr` があるところまでの行が再生される
- `!blk(a,b,c..)` : b, c と引数を二つ以上に増やすことで、ブロック内の任意の文字列を差し替えることができる
