...
impl TickGen {
    ...
    pub fn gen_tick(&mut self, crnt_time: Instant) -> bool {
        ...
        if self.rit_state {
            // rit.
            if let Some(tick_in_msr) = self.ritgen.calc_tick_rit(
                crnt_time,
                self.bpm_start_time,
                ...
                self.crnt_msr += tick_in_msr / self.tick_for_onemsr;
                self.crnt_tick_inmsr = tick_in_msr % self.tick_for_onemsr;
            } else {
                // rit 終了
                self.rit_state = false;
                self.crnt_msr = self.beat_start_msr + self.ritgen.rit_bar + 1;
    ...
    pub fn get_real_bpm(&self) -> i16 {
        self.bpm - self.ritgen.delta_bpm
    }
    ...
    pub fn start_rit(&mut self, start_time: Instant, ratio: i32, bar: i32, target_bpm: i16) {
        if ratio < 100 && !self.rit_state && !self.fermata_state {
            self.ritgen.set_rit(ratio, bar, self.bpm as f32);
        }
        self.rit_state = true;
        self.beat_start_msr = self.crnt_msr;
...
impl RitGen {
    ...
    pub fn set_rit(&mut self, ratio: i32, bar: i32, bpm: f32) {
    ...
    pub fn calc_tick_rit(
        &mut self,
        crnt_time: Instant,
        bpm_start_time: Instant,
        tick_for_onemsr: i32,
        original_bpm: i16,
        bpm_start_tick: i32,
    ) -> Option<i32> {
        // output: self.crnt_msr の更新
        let tick_from_rit_starts =
            self.calc_addup_tick_rit(crnt_time, bpm_start_time, original_bpm) + bpm_start_tick;
        if tick_for_onemsr * (self.rit_bar + 1) < tick_from_rit_starts {
            // reached last bar, and stop rit.
            self.rit_bar = 0;
            self.delta_bpm = 0;
            None
        } else {
            let r_msr = tick_from_rit_starts / tick_for_onemsr;
            let mut r_tick_inmsr = tick_from_rit_starts % tick_for_onemsr;
            if r_msr > self.rit_bar_count {
                // 小節線を超えたとき
                self.rit_bar_count += 1;
                r_tick_inmsr += tick_for_onemsr;
            }
            Some(r_tick_inmsr)
        }
    }
    fn calc_addup_tick_rit(
        &mut self,
        crnt_time: Instant,
        bpm_start_time: Instant,
        original_bpm: i16,
        ...
        let start_time = (crnt_time - bpm_start_time).as_secs_f32();
        let time_to0 = self.t0_time - start_time;
        self.delta_bpm = (self.delta_tps * start_time / 8.0) as i16;
        let addup_tick: i32;
        if original_bpm - self.delta_bpm > MINIMUM_TEMPO {
            ...
            self.delta_bpm = original_bpm - MINIMUM_TEMPO;
