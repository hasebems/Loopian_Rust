...
* noteは移動ドで表現する
    ...
    - `d=m=s` or `dms` : 同時演奏 ('='で結ぶか、各音を連続して書く)
    - `d*4` : ドを４回連続して発音
...
* Dynamic Pattern は、Phrase の代わりに、実際の音程を支持せず、和音やアルペジオ演奏を自動生成する
    - 内部的には Phrase と見做され、Dynamic Pattern が指定されたら、Phrase は上書きされる
...
* `ARP(a,b,c)` とあった場合、各パラメータは以下の意味になる
    - a: 音価を表す。h,q,e,v,w およびその付点を選ぶことができる(default:q)
    - b: u は上昇音形。dは下降、ud(上昇->下降),du(下降->上昇)も指定できる。
    - c: 最低音の位置(-6 - 7)(default:0)
...
- プロンプトに `!load.`*filename* (あるいは `!l.`*filename*) と書くと、指定したファイルがロードされる
    ...
    - `!msr(20)` と書かれた行では、それ以降の行が20小節の1拍目に間に合うように再生されることを表す
        - 実際には、19小節目の最後の拍くらいで再生される
    ...
    - `!load` のみの入力でも、前回のファイル名を覚えており、前回と同じファイルがロードされる。
        - `!load.blk(a)` というように、真ん中のファイル名を省略することも可能
    - `!load.`*filename*`.msr(n)` とすると、ファイル内の行の冒頭が `!msr(n)` とある行から再生が開始される。
