...
- ある音程のオクターブを設定する際、相対距離と絶対距離の二つの考え方がある
    - 相対距離指定、近い方の音程に行く場合、何も表記しない。近くない場合、以下のように指定する
        - `-d` : 下の方のド
        - `+d` : 上の方のド
        - `--d`: 二つ下
        - `++d`: 二つ上
    - 絶対距離指定の場合、`d`から`t`までが同じオクターブに含まれ、それより上の音程を鳴らす場合 `+` を、同様に下の音程を鳴らす場合、`-` を音程の前に付ける
