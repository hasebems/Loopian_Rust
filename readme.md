...
`quit` あるいは `q` をプロンプトに入力し、returnするとアプリが終了
...
* `left1` あるいは `L1` : 入力パートをleft1 にする
* `left2` あるいは `L2` : 入力パートをleft2 にする
* `right1` あるいは `R1` : 入力パートをright1 にする
* `right2` あるいは `R2` : 入力パートをright2 にする
* `panic` : 今鳴っている音を消音する
...
* noteは移動ドで表現する
    ...
    - `|` あるいは `/` : 小節区切り。区切りが連続すると休符が省略されたとみなす
    - `d=m=s` あるいは `d_m_s` : 同時演奏
        - `dms` と =,_ を省略することも可能
    - `d*4` : ドを４回連続して発音
...
* noteの音価表現
    ...
    - `"` or `v`:十六分音符
    - `'"`:三十二分音符
        - 一度上記の音価を書けばこの指定が基準音価となり、新しい音価を書かない限りはこの音価が継続する
