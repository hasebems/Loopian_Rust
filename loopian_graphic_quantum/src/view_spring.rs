...
impl Spring {
	...
	fn draw_spring(&self, draw: &Draw, anchor: Point2, mass_center: Point2, spring_color: Rgba, stroke: f32) {
	...
	fn draw_mass(&self, draw: &Draw, center: Point2, radius: f32, mass_color: Rgba, highlight_color: Rgba) {
		draw.ellipse()
...
impl NoteObj for SpringNote {
	...
	fn disp(&self, draw: Draw, crnt_time: f32, rs: Resize) {
		...
		let x = map_range(self.part as f32, 0.0, 3.0, -rs.get_full_size_x() * 0.44, rs.get_full_size_x() * 0.44);
		draw.ellipse()
